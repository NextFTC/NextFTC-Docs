...
Coroutine commands use Kotlin `suspend` functions and can't be written from Java.
Java code can still schedule and compose the resulting `Command` like any other.
</Aside>
...
The coroutine command isn't done until the body has returned *and* every forked command has finished.
...
don't write anything blocking. A loop with no suspension point in it hangs the robot.
...
`Thread.sleep` has the same problem. Use `wait(seconds)`.
